...
// ── Schedule Runner + Automation Rule Polling ──
// Polls every 30 seconds:
//   schedule_runner.php    → fires sprayer schedules
//   automation_runner.php  → checks sensor-based rules (mist, fan, heater, exhaust)
(function() {
  console.log('[Runners] Starting polling every 30 seconds');

  let pageNeedsRefresh = false;

  // ── Sprayer Schedule Runner ──
  async function pollScheduleRunner() {
    try {
      const response = await fetch('schedule_runner.php');
      const data = await response.json();
      if (data.fired > 0) {
        console.log(`[Schedule Runner] Fired ${data.fired} schedule(s)`);
  ...
  // ── Automation Rule Runner (sensor-based: mist, fan, heater, exhaust) ──
  async function pollAutomationRunner() {
    try {
      const response = await fetch('automation_runner.php');
      const data = await response.json();
      if ((data.fired > 0) || (data.turned_off > 0)) {
        console.log(`[Automation Runner] ON: ${data.fired}, OFF: ${data.turned_off}`, data.actions);
        pageNeedsRefresh = true;
      }
    } catch (error) {
  ...
  // ── Run both, then refresh log if anything happened ──
  async function runAll() {
    pageNeedsRefresh = false;
    await Promise.all([pollScheduleRunner(), pollAutomationRunner()]);
    if (pageNeedsRefresh) {
