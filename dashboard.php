...
// Redesigned UI – retains all original functionality
...
$mushroomRecords = [];
while ($row = $mushroomResult->fetch_assoc()) $mushroomRecords[] = $row;
$mushroomStmt->close();
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .upload-zone label { font-size: 13px; font-weight: 600; white-space: nowrap; }
    ...
    #uploadStatus { font-size: 12px; color: var(--muted); margin-top: 6px; }
...
<main class="main">
  ...
  <div class="grid">
    ...
    <!-- ── Live Environment (col 4) ── -->
    <div class="card col-4">
      <div class="card-header">
        <div class="card-title">
          <span class="icon icon-green"><i class="fas fa-leaf"></i></span>
          Live Environment
        </div>
        ...
        <div class="gauges-row">
          <div class="gauge-item">
            <div class="gauge-wrap">
              ...
              <div class="gauge-val" id="tempValue">—</div>
            </div>
          ...
          <div class="gauge-item">
            <div class="gauge-wrap">
              ...
              <div class="gauge-val" id="humValue">—</div>
            </div>
    ...
    <!-- ── Device Control (col 4) ── -->
    <div class="card col-4">
      <div class="card-header">
        <div class="card-title">
          <span class="icon icon-blue"><i class="fas fa-sliders"></i></span>
          Device Control
        </div>
        ...
        <div id="manualControls" style="display:none;">
          <div class="devices">
            ...
            <div class="device-row">
              ...
              <span class="device-time muted" id="last_<?= $id ?>">—</span>
    ...
    <div class="card col-4">
      <div class="card-header">
        <div class="card-title">
          <span class="icon icon-red"><i class="fas fa-bell"></i></span>
          Alerts
        </div>
    ...
    <div class="card col-6">
      <div class="card-header">
        ...
        <span style="font-size:11px;color:var(--muted);"><?= date('F Y') ?></span>
      </div>
      <div class="card-body" style="padding:0;">
        <?php if (empty($mushroomRecords)): ?>
          <div class="empty-state"><i class="fas fa-seedling"></i><span>No records for this month yet.</span></div>
        <?php else: ?>
          <table class="rec-table">
            <thead><tr><th>Date</th><th>Count</th><th>Stage</th><th>Notes</th></tr></thead>
            <tbody>
              <?php foreach ($mushroomRecords as $r): ?>
              <tr>
                <td><?= htmlspecialchars(date('M j', strtotime($r['record_date']))) ?></td>
                <td><span class="count-badge"><?= htmlspecialchars($r['mushroom_count']) ?></span></td>
                <td><span class="stage-badge"><?= htmlspecialchars($r['growth_stage']) ?></span></td>
                <td style="color:var(--muted);font-size:12px;"><?= htmlspecialchars($r['notes'] ?: '—') ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
    ...
    <!-- ── Image Analysis (col 6) ── -->
    <div class="card col-6">
      <div class="card-header">
        <div class="card-title">
          <span class="icon icon-blue"><i class="fas fa-camera"></i></span>
          Image Analysis
        </div>
        <button class="info-btn" id="imageInfoIcon"><i class="fas fa-info"></i></button>
      </div>
      <div class="card-body">
        <div class="upload-zone">
          <label>Upload Image</label>
          <input type="file" id="imageInput" accept="image/*">
          <button class="upload-btn" id="analyzeBtn">Analyze & Save</button>
        </div>
        <div id="uploadStatus"></div>
        <div class="img-grid" id="mushroomImageGrid">
          <div class="empty-state" id="noImages" style="grid-column:1/-1;">
            <i class="fas fa-camera"></i>
            <span>No images analyzed yet.</span>
          </div>
        </div>
      </div>
    </div>
...
<!-- Image Info -->
<div class="modal-backdrop" id="imageInfoModal">
  <div class="modal">
    <button class="modal-close" data-close="imageInfoModal">&times;</button>
    <h3>Image Analysis</h3>
    <p>Upload a mushroom photo to analyze size, harvest readiness, and confidence score. Results are saved for review.</p>
    <h4>Readiness Labels</h4>
    <div class="legend-row"><span class="leg-dot" style="background:var(--green)"></span><span>Ready to harvest</span></div>
    <div class="legend-row"><span class="leg-dot" style="background:var(--amber)"></span><span>Almost ready</span></div>
    <div class="legend-row"><span class="leg-dot" style="background:var(--blue)"></span><span>Not ready</span></div>
    <div class="legend-row"><span class="leg-dot" style="background:var(--red)"></span><span>Overripe</span></div>
  </div>
</div>
...
function makeGauge(id, color) {
  return new Chart($$(id).getContext('2d'), {
    ...
    options: { cutout: '75%', rotation: -90, circumference: 180, responsive: true, maintainAspectRatio: false,
...
function setGaugeStatus(el, cls, text) {
  el.className = 'gauge-status ' + cls;
  el.textContent = text;
}
...
async function loadLive() {
  try {
    ...
    ['tempValue','humValue'].forEach(id => $$(''+id) && ($$(''+id).textContent = '—'));
    $$('time').textContent = 'Offline';
    setGaugeStatus($$('tempNote'), 'gs-offline', 'Offline');
    setGaugeStatus($$('humNote'),  'gs-offline', 'Offline');
    [tempGauge, humGauge].forEach(g => {
      g.data.datasets[0].data = [0, 100];
      g.data.datasets[0].backgroundColor = ['#e5e7eb', '#f0f2f5'];
      g.update();
    });
...
function renderAlerts(msgs) {
  ...
  if (!msgs.length || (msgs.length === 1 && msgs[0] === '')) {
    list.innerHTML = '<div class="alert-item alert-ok"><i class="fas fa-check-circle"></i> No alerts</div>';
    return;
  }
  if (msgs[0] === 'Device offline') {
    list.innerHTML = '<div class="alert-item alert-warn"><i class="fas fa-wifi"></i> Device offline</div>';
    return;
  }
  msgs.forEach(m => {
    const d = document.createElement('div');
    d.className = 'alert-item alert-err';
    d.innerHTML = `<i class="fas fa-triangle-exclamation"></i> ${m}`;
    list.appendChild(d);
  });
}
...
$$('analyzeBtn').addEventListener('click', async function() {
  ...
  if (!input.files.length) { status.textContent = 'Please select an image first.'; return; }
  const form = new FormData();
  form.append('image', input.files[0]);
  status.textContent = 'Analyzing…';
  try {
    const r = await fetch('analyze_mushroom.php', { method: 'POST', body: form });
    const d = await r.json();
    status.textContent = d.message || 'Done.';
    if (d.result) addImageCard(d.result);
...
function addImageCard(data) {
  ...
  $$('noImages')?.remove();
  const statusMap = {
    'ready':     ['harvest-ready',  'Ready'],
    'almost':    ['harvest-almost', 'Almost Ready'],
    'not-ready': ['harvest-not',    'Not Ready'],
    'overripe':  ['harvest-over',   'Overripe'],
  };
  const [cls, label] = statusMap[data.status] || ['harvest-not', data.status || '—'];
  const card = document.createElement('div');
  card.className = 'img-card';
  card.innerHTML = `
    <img src="${data.image_url || '#'}" alt="Mushroom">
    <div class="img-info">
      <div class="img-size">${data.size || '—'}</div>
      <span class="status-pill ${cls}" style="font-size:11px;padding:2px 8px;">${label}</span>
      <div class="img-ts">${data.timestamp || ''}</div>
      <div class="img-conf">Confidence: ${data.confidence || '—'}</div>
    </div>`;
  grid.prepend(card);
}
...
function bindModal(triggerIds, modalId) {
  ...
  triggerIds.forEach(tid => {
    const el = $$(tid);
    if (el) el.addEventListener('click', () => modal.classList.add('show'));
  });
...
bindModal(['imageInfoIcon'],  'imageInfoModal');
</script>
</body>
</html>